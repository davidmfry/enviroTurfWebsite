...
class schoolPage extends React.Component
{
    ...
    componentDidMount ()
    {
        const { state, schoolName} = this.props.url.query;
        firebase.database().ref("states").child(`${state}`).child('schools').on('value', (snapshot) => {
            _.map(snapshot.val(), (school) => {
                if (school.schoolName === schoolName)
                {
                    this.setState({schoolImageList: school.imageUrls.split(',')})
                }
            })
...
//     }
    ...
    render()
    {
        ...
        return (
            <Layout>
                <SchoolItem name={this.props.url.query.schoolName} images={this.state.schoolImageList}/>
            </Layout>
        )
    }
